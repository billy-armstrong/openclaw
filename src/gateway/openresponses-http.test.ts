...
describe("OpenResponses HTTP API (e2e)", () => {
  ...
  it("replays strict SDK output and preserves structured tool data without fetching it", async () => {
    ...
    expect(opts.message).toContain("Checking the weather.");
    expect(opts.message).toContain(JSON.stringify(structuredOutput));
    expect(opts.message).toContain("Summarize the result.");
  ...
  it.each([
    ...
    },
    {
      ...
      item: {
        ...
        name: "get_weather",
        arguments: "{}",
        caller: { type: "direct", caller_id: "untrusted-program-escalation" },
      },
    },
    {
      name: "an unknown nested structured-output field",
      item: {
        type: "function_call_output",
        call_id: "call_invalid_1",
        output: [
    ...
    const res = await postResponses(enabledPort, {
      model: "openclaw",
      input: [item, { type: "message", role: "user", content: "Reject invalid replay." }],
    });
    const body = await res.text();
  ...
  it.each([false, true])(
    ...
    async (stream) => {
      ...
      agentCommandMock.mockClear();
      agentCommandMock.mockResolvedValueOnce({
        payloads: [{ text: privatePayload, isError: true }],
        meta: {
      ...
      if (stream) {
        ...
        const failed = parseSseData(findSseEvent(events, "response.failed")) as {
      ...
      expect(body).not.toContain(privatePayload);
  ...
  it.each([
    {
      name: "same normalized user",
      firstUser: "  alice  ",
      secondUser: "alice",
      reuses: true,
    },
    { name: "different user", firstUser: "alice", secondUser: "bob", reuses: false },
    { name: "omitted named user", firstUser: "alice", secondUser: undefined, reuses: false },
    { name: "anonymous user", firstUser: undefined, secondUser: undefined, reuses: true },
  ])("scopes previous responses by $name", async ({ firstUser, secondUser, reuses }) => {
    agentCommandMock.mockClear();
    agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "First turn." }] } as never);
    const firstResponse = await postResponses(enabledPort, {
      model: "openclaw",
      ...(firstUser === undefined ? {} : { user: firstUser }),
      input: "first turn",
    });
    expect(firstResponse.status).toBe(200);
    const firstJson = (await firstResponse.json()) as { id?: string };
    ...
    const secondResponse = await postResponses(enabledPort, {
      model: "openclaw",
      ...(secondUser === undefined ? {} : { user: secondUser }),
      previous_response_id: firstJson.id,
      input: "second turn",
    });
    expect(secondResponse.status).toBe(200);
    const secondSessionKey = requireSessionKey(
      (firstAgentOpts(1) as { sessionKey?: string }).sessionKey,
      "second response",
    );
    expect(secondSessionKey === firstSessionKey).toBe(reuses);
    if (secondUser) {
      expect(secondSessionKey).toContain(`openresponses-user:${secondUser}`);
    }
    await ensureResponseConsumed(secondResponse);
  });
